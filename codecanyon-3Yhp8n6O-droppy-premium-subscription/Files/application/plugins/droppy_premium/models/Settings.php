...
class PremiumSettings {
    ...
    function updatePremium($version = false, $current_version = false) {
        if($version !== false) {
            ...
            if($current_version < '2.1.7') {
                run_sql_file(dirname(__FILE__) . '/../updates/update_2.1.7.sql', $this->db);
            }
            if($current_version < '2.1.8') {
                run_sql_file(dirname(__FILE__) . '/../updates/update_2.1.8.sql', $this->db);
            }
            if($current_version < '2.1.9') {
                run_sql_file(dirname(__FILE__) . '/../updates/update_2.1.9.sql', $this->db);
            }
            if($current_version < '2.2.0') {
                run_sql_file(dirname(__FILE__) . '/../updates/update_2.2.0.sql', $this->db);
            }
            if($current_version < '2.2.1') {
                run_sql_file(dirname(__FILE__) . '/../updates/update_2.2.1.sql', $this->db);
            }
