...
&lt;/ul&gt;</pre>
            ...
            <div class="mb-3">
                ...
                <div class="col-sm-10">
                    <input type="number" step=".01" class="form-control" name="plan_price" placeholder="Plan price, currency is set on General settings page" value="<?php echo $plan['plan_price'] ?>">
            ...
            <div class="mb-3">
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="plan_password_enabled">
                        <option value="true" <?php if($plan['plan_password_enabled'] == 'true') { echo 'selected'; } ?>>Allow for users using this plan</option>
                        <option value="false" <?php if($plan['plan_password_enabled'] == 'false') { echo 'selected'; } ?>>Block for users using this plan</option>
                    </select>
                    <p><i>Select if the password function should be enabled for premium users.</i><br>
                        <i>Note that if you want this feature to be disabled for basic users you'll have to change this in the base upload settings of Droppy.</i></p>
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label">Share option enabled (Only premium)</label>
                <div class="col-sm-10">
                    <select class="form-control" name="plan_share_enabled">
                        <option value="true" <?php if($plan['plan_share_enabled'] == 'true') { echo 'selected'; } ?>>Allow for users using this plan</option>
                        <option value="false" <?php if($plan['plan_share_enabled'] == 'false') { echo 'selected'; } ?>>Block for users using this plan</option>
                    </select>
                    <p><i>Select if the share function should be enabled for premium users (the option in the upload form to choose between email or link).</i><br>
                        <i>Note that if you want this feature to be disabled for basic users you'll have to change this in the base upload settings of Droppy.</i></p>
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label">Destruct option enabled (Only premium)</label>
                <div class="col-sm-10">
                    <select class="form-control" name="plan_destruct_enabled">
                        <option value="true" <?php if($plan['plan_destruct_enabled'] == 'true') { echo 'selected'; } ?>>Allow for users using this plan</option>
                        <option value="false" <?php if($plan['plan_destruct_enabled'] == 'false') { echo 'selected'; } ?>>Block for users using this plan</option>
                    </select>
                    <p><i>Select if the destruct option should be enabled for premium users (the option in the upload form to choose between auto-destroy on download or not).</i><br>
                        <i>Note that if you want this feature to be disabled for basic users you'll have to change this in the base upload settings of Droppy.</i></p>
                </div>
            </div>
            ...
            <div class="mb-3">
                <label class="form-label">Plan active</label>
                <div class="col-sm-10">
                    <select class="form-control" name="plan_active">
                        <option value="true" <?php if($plan['plan_active'] == 'true') { echo 'selected'; } ?>>Yes</option>
                        <option value="false" <?php if($plan['plan_active'] == 'false') { echo 'selected'; } ?>>No</option>
                    </select>
                    <p><i>Choose if the plan is active, if not the plan will not be shown in the frontend. Subscribers that already have the plan are still subscribed to the plan.</i></p>
                </div>
            </div>
